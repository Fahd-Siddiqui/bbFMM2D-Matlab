...
function [ R ] = get_Transfer_From_Parent_To_Children( N, nChebNodes, ...
                                              location, center, radius, TNode )
% Obtains interpolation operator, which interpolates information from Chebyshev
% nodes of parent to Points in children
...
for k = 1 : 2
    ...
    Transfer{k} = H2_2D_Tree.get_Standard_Chebyshev_Polynomials(nChebNodes, ...
                                                     N , standlocation(: , k));
...
for i = 1 : nChebNodes
    for j = 1 : nChebNodes
        R(: , i + nChebNodes * (j - 1)) = Transfer{1}(: , i) .* ...
                                          Transfer{2}(: , j);
    end
end
